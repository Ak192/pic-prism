...
import useupload from '../../hooks/UseUploadHook';
...
const ImageAdd = () => {
  ...
  try{
    ...
    if(title.trim===" " || price.trim ===" ")return toast.error("please fill all the field !")
      const {public_id,secure_url}=useupload({image,onUploadProgress});
    const res= await axios.post(import.meta.env.VIT_API_URL +"/post/create",
  ...
  return (
    ...
    {/*show a progress Bar  */}
     ...
     <div className="flex flex-col">
      ...
      <button type="submit" className='bg-slate-900 text-white py-1 px-3 mt-1 border rounded-lg '>submit</button>
     </div>
    </form>
    </div>
  )
